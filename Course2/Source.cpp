...
bool ProcString(String^ Line, List% pHead, List% pTail)
{
	...
	{
		MessageBox::Show("Недопустимый формат площади страны " + words[0], "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
		return false;
	}
	...
	{
		MessageBox::Show("Недопустимый формат численности картин страны " + words[0], "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
		return false;
	}
